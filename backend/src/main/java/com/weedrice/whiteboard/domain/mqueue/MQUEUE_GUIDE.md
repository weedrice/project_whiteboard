# Mqueue (메시지 큐) 도메인 가이드

`mqueue` 도메인은 이메일, PUSH, SMS 등 사용자에게 발송되는 각종 메시지를 **비동기적**으로 처리하기 위한 큐(Queue) 시스템을 담당합니다. 이는 실제 사용자 간의 쪽지(`message` 도메인)와는 다른 기능입니다.

## 1. 주요 기능 및 로직

`MqueueService`와 `MqueueScheduler`를 통해 다음과 같은 핵심 비즈니스 로직을 처리합니다.
...
`mqueue` 도메인은 사용자에게 직접 노출되는 API 엔드포인트가 없습니다. 다른 서비스 계층에서 `MqueueService`를 호출하여 사용합니다.

## 3. 관련 DB 테이블

`mqueue` 도메인은 주로 다음 테이블과 상호작용합니다.

| 테이블명          | 엔티티          | 설명             |
| :---------------- | :-------------- | :--------------- |
| `message_queue`   | `MessageQueue`  | 발송 대기 메시지 정보 |

이 문서는 `mqueue` (메시지 큐) 도메인의 기능을 이해하고 개발하는 데 도움을 주기 위해 작성되었습니다.
