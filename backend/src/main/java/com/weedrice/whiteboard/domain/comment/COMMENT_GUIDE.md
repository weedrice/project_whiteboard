...
- 베스트 댓글: 게시글별 좋아요 상위 댓글을 별도 목록으로 제공합니다.
- 댓글 작성: 부모 유효성 검증 후 저장, 게시글 댓글 수 증가, 버전 기록, Closure Table 적재, `POINT_COMMENT_CREATE_REWARD` 전역 설정(기본 10)에 따른 작성 보상 부여, 부모/게시글 작성자에게 알림 발행.
- 댓글 수정/삭제: 본인 소유 검증, 수정/삭제 시 원본 내용을 버전으로 남기고 댓글 수를 조정하며, 삭제 시 해당 댓글에 실제 적립된 작성 보상을 포인트 이력 기준으로 회수.
...
| `GET` | `/api/v1/posts/{postId}/comments/best` | 베스트 댓글 목록 조회 |
