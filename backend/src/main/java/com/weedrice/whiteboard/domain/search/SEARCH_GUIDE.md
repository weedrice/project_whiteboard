...
- 통합 검색: 게시글/댓글/사용자/스페이스를 그룹별 페이지 형태로 조회합니다. 기본 크기는 5건이고
  요청 크기는 최대 100건으로 제한합니다. 전역 스페이스 결과는 활성·공개·목록 노출 스페이스로
  제한하며, 차단 사용자 콘텐츠를 제외하고 게시글의 이미지 존재 여부를 계산합니다. 게시글 전용
  필터를 사용하면 호환되지 않는 댓글/사용자/스페이스 그룹은 빈 결과로 반환합니다.
