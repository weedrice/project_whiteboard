...
## 1. 주요 기능 및 로직
- 게시글 목록: 노드/카테고리/최소 좋아요 필터로 조회하며 차단 사용자 글은 제외, 썸네일 여부(hasImage) 계산.
- 트렌드/최신: 최근 24시간 인기 글, 노드별 최신 글 15개, 태그별 글을 제공하고 좋아요/스크랩/구독 여부를 함께 반환합니다.
...
| Method | URI | 설명 |
| :----- | :-------------------------------------------------- | :---------------------------------------- |
| `GET` | `/api/v1/boards/{boardUrl}/posts` | 노드 게시글 목록 조회(필터/키워드 기록 포함) |
