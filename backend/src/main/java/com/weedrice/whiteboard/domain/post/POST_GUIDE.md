...
- 트렌드/최신: 기간별 인기 글, 스페이스별 최신 글 15개, 태그별 글을 제공하고 좋아요/스크랩/구독 여부를 함께 반환합니다. 트렌딩 `period`는 `24h`(기본), `7d`, `30d`를 지원하며 알 수 없는 값은 24시간으로 처리합니다.
  - **트렌딩 게시글 노출 조건**: 선택 기간 내 작성된 비삭제·비블라인드·비공지·비밀글 아님 조건의 글로, 활성·공개·목록 노출 스페이스에 속하고 차단 사용자가 작성하지 않아야 합니다. 활성(또는 상태가 없는 기존) 첨부 이미지, 본문 `<img` 또는 `<iframe` 중 하나를 포함해야 합니다.
  - **정렬 기준**: 점수 = (조회수 × 1) + (좋아요 수 × 10). 점수, 작성 시각, 게시글 ID 내림차순으로 정렬합니다.
  - **썸네일 URL 결정**: 첨부 이미지가 있으면 파일의 thumbnail URL을 사용하고, 없으면 본문에서 썸네일 URL 허용 정책을 통과한 첫 번째 이미지를 사용합니다.
- 게시글 상세: 조회수 증가 및 ViewHistory 갱신, 태그·이미지 URL·좋아요/스크랩 여부·관리자 여부 포함 응답.
- 작성/수정/삭제: 카테고리 권한/공지 작성 권한 검증, 태그·첨부 파일 연결, 버전 기록, 작성 시 `POINT_POST_CREATE_REWARD` 전역 설정(기본 50)에 따른 보상 지급, 삭제 시 해당 게시글에 실제 적립된 작성 보상을 포인트 이력 기준으로 회수.
...
| `GET` | `/api/v1/posts/trending` | 기간별 인기 게시글 목록 (`24h`/`7d`/`30d`, 기본 `24h`) |
...
- 보호 상태 예약은 사용자별 100개로 제한한다. 예정 시각에서 30일이 지난 `FAILED` 예약은 자동 취소해 초안 보호와 예약 파일 참조를 해제한다.
- 서버 초안 정리는 도메인 잠금을 사용해 다중 인스턴스에서 한 번만 수행하며, 브라우저 복구본과 삭제 표식은 90일 뒤 정리한다.
