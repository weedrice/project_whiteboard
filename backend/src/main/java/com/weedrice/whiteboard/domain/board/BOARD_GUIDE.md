...
- **카테고리 관리 (`getActiveCategories`, `createCategory` 등):**
  - 특정 게시판에 속한 카테고리를 조회, 생성, 수정, 삭제합니다. (생성/수정/삭제는 관리자 권한 필요)
...
| `PUT`    | `/api/v1/boards/{boardId}`               | 게시판 수정              | 생성자/관리자 |
| `DELETE` | `/api/v1/boards/{boardId}`               | 게시판 삭제              | 생성자/관리자 |
