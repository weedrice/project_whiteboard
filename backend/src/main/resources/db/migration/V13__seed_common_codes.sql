-- Idempotent seed migration derived from backend/DATABASE_INIT.sql.

WITH ranked_code_details AS (
    SELECT
        id,
        ROW_NUMBER() OVER (
            PARTITION BY type_code, code_value
            ORDER BY id ASC
        ) AS row_num
    FROM common_code_details
)
DELETE FROM common_code_details detail
USING ranked_code_details ranked
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'uk_code_details_value'
          AND conrelid = 'common_code_details'::regclass
    ) THEN
        ALTER TABLE common_code_details
            ADD CONSTRAINT uk_code_details_value
            UNIQUE (type_code, code_value);
    END IF;
END
$$;
...
ON CONFLICT (type_code, code_value) DO UPDATE SET
    ...
    is_active = EXCLUDED.is_active,
    modified_at = NOW();
SELECT setval(pg_get_serial_sequence('common_code_details', 'id'), COALESCE((SELECT MAX(id) FROM common_code_details), 1), true);
