...
(참고: 조회수 등은 Redis 등을 활용한 Write-Back(지연 쓰기) 방식을 권장)
...
(참고: 동시성 이슈 방지를 위해 메시지 큐 또는 DB Lock 전략 고려)
...
**[구현 가이드] Notification Type Mapping**
| 이벤트 | notification_type | source_type | 설명 |
|---|---|---|---|
| 게시글에 댓글 작성 | COMMENT | POST | 내 글에 댓글이 달림 |
| 댓글에 대댓글 작성 | REPLY | COMMENT | 내 댓글에 대댓글이 달림 |
| 게시글 좋아요 | LIKE | POST | 내 글에 좋아요 |
| 댓글 좋아요 | LIKE | COMMENT | 내 댓글에 좋아요 |
| 사용자 멘션 | MENTION | POST/COMMENT | 글/댓글에서 나를 언급 |
...
| 기능 ID | ADMIN-007 |
| 설명 | 게시글 삭제 또는 복구 |
| 선행조건 | 슈퍼 관리자(SUPER) 또는 해당 게시판의 담당 관리자(BOARD_ADMIN) |
...
| 설명 | 발송 대기 메시지 처리 |
| 주기 | 1분마다 |
| 처리로직 | 1. message_queue 조회 (status = PENDING)<br>2. 발송 처리 (비동기 Worker Thread 또는 별도 발송 서버에서 병렬 처리 권장)<br>3. status UPDATE (SENT/FAILED)<br>4. 실패 시 retry_count + 1 |
