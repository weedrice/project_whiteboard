...
하나이고 읽기·쓰기가 모두 된다. 두 이름을 함께 내보내던 필드 51개를 이 방식으로 정리했다.
...
공개 프로필 댓글 활동은 조회자와 프로필 주인 사이의 차단뿐 아니라 원글 작성자와의 양방향 차단도 반영하며, 제외한 댓글은 페이지 전체 개수에도 포함하지 않는다.
`PUT /api/v1/users/me`의 프로필 이미지 변경 비용이 0이면 첫 무료 변경권 사용 후에도 포인트 차감 없이 변경한다.
...
세션 API의 현재 기기는 refresh 쿠키가 아니라 검증된 access token의 session family로 식별한다.
`refreshToken` 쿠키의 `/api/v1/auth` 경로는 유지하며, `/users/me/sessions` 요청에 이 쿠키가 없어도
현재 세션 표시와 다른 세션 폐기가 동작한다. 인증된 사용자와 일치하는 세션 정보를 확인할 수 없으면
`401 Unauthorized`(`C004`)로 거절하며 전체 세션 폐기로 대체하지 않는다. 선택한 세션이 현재 세션이면
응답에서 refresh 쿠키를 만료시키고, 프런트엔드도 현재 로그인 상태를 즉시 정리한다.

### Boards And Posts

게시판 관리자 권한은 공개·활성 여부와 무관하게 해당 게시판의 활성 관리자 배정으로 판정한다.
공개·활성 게시판의 관리자도 게시글 목록에서 비밀글을 조회할 수 있으며, 상세 응답의 `board.isAdmin`에
관리 권한이 반영된다. 비관리자의 비밀글 접근 제한은 유지한다.
...
투표 요청은 기존 선택을 요청의 선택지 목록으로 교체한다. 같은 선택의 재전송이나 기존 선택을 포함한
복수 선택 변경도 중복 표 없이 처리한다. 내 시리즈 목록 조회는 계정 상태를 검증하며 쓰기 잠금을 획득하지 않는다.
...
- 사용자별 `SCHEDULED`, `PUBLISHING`, `FAILED` 예약은 합계 100개로 제한하며 초과 시 `P006(SCHEDULED_POST_LIMIT_EXCEEDED)`를 반환한다.
- 발행 예정 시각에서 30일이 지난 `FAILED` 예약은 자동으로 취소 상태로 전환해 초안과 예약 파일 참조 보호를 해제한다.
...
댓글 명령은 보드·게시글 잠금 이후의 최신 삭제·공개·블라인드 상태로 접근을 검증한다.
댓글 수정·삭제도 잠금 이후 최신 댓글 상태로 권한과 수정 가능 여부를 판단한다.
댓글 작성과 수정 모두 후보 목록에서 선택한 사용자의 ID를 `mentionedUserIds`로 전달한다.
수정 시 기존 멘션은 보존하고, 제거된 멘션은 목록에서 제외하며 새로 추가된 대상에게만 멘션 알림을 생성한다.
일반 텍스트로 입력한 `@닉네임`만으로 대상 사용자를 자동 확정하지 않는다.
잠금 대기 중 변경된 좋아요 수를 덮어쓰지 않으며, 삭제·블라인드된 댓글은 기존 접근 정책에 따라 거부한다.
...
최종 이미지 수 검증, 신규 파일 연결, 이미지/메타데이터 변경을 하나의 트랜잭션으로 처리한다.
삭제한 이미지는 팩의 이미지 목록에서 제거하되, 기존 댓글의 이미지 URL이 계속 동작하도록 활성 파일과 팩 연결을 보존한다.
보존 파일에도 기존 팩의 공개 여부·소유자·구매자 읽기 권한을 적용한다. 팩 전체 삭제는 구매 이력 검증 후
현재 목록에서 제거된 이미지 파일까지 함께 삭제 예약한다. 이미 삭제된 파일을 복원하는 처리는 포함하지 않는다.
...
문의 공개 메시지 이미지는 문의 작성자 또는 사용 가능한 슈퍼관리자만 접근할 수 있다. 내부 메모 이미지는
사용 가능한 슈퍼관리자 전용이며, 슈퍼관리자가 문의 작성자인 경우에도 같은 권한을 유지한다.
