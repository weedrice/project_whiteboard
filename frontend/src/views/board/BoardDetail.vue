...
const posts = computed(() => {
    const p = postsData.value?.content || []
  ...
  <div>
    ...
    <div v-else-if="board">
      ...
      <div class="bg-white dark:bg-gray-800 shadow rounded-lg mb-6 p-6 transition-colors duration-200">
        <div class="flex items-start">
          <router-link :to="`/board/${board.boardUrl}`" class="flex-shrink-0 mr-6 cursor-pointer">
              ...
              <div v-else class="h-20 w-20 rounded-full bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center">
                <span class="text-indigo-600 dark:text-indigo-400 font-bold text-3xl">{{ board.boardName[0] }}</span>
              </div>
          </router-link>
      ...
      <div class="bg-white dark:bg-gray-800 shadow rounded-lg transition-colors duration-200">
        ...
        />
        <!-- Pagination -->
        <div class="px-4 py-3 border-t border-gray-200 dark:border-gray-700 flex justify-center" v-if="totalPages > 1">
            <Pagination 
                :currentPage="page" 
      ...
      <div class="mt-4 px-4 py-4 sm:px-6 bg-gray-50 dark:bg-gray-800 rounded-lg relative flex justify-center items-center transition-colors duration-200">
        <div class="flex max-w-lg w-full">
            <select v-model="searchType" class="block pl-3 pr-8 py-2 text-base border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white mr-2 cursor-pointer shadow-sm transition-colors duration-200">
            ...
            <div class="relative flex-grow">
                ...
                <div v-if="isSearching" class="absolute inset-y-0 right-2 pr-3 flex items-center">
                    <button type="button" @click="clearSearch" class="text-gray-400 hover:text-gray-500 dark:hover:text-gray-300">
                    <X class="h-5 w-5" />
                    </button>
                </div>
            </div>
