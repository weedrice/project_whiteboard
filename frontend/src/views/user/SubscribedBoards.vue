<template>
  <div class="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden transition-colors duration-200">
    <div class="px-4 py-5 sm:px-6">
        <h3 class="text-lg font-medium leading-6 text-gray-900 dark:text-white">{{ $t('user.subscriptions.title') }}</h3>
    </div>
    ...
    >
      <template #item="{ element: board }">
        <li class="px-4 py-4 sm:px-6 hover:bg-gray-50 dark:hover:bg-gray-700 flex justify-between items-center bg-white dark:bg-gray-800 transition-colors duration-200">
            <div class="flex items-center flex-1 cursor-pointer">
                <div class="handle mr-4 cursor-move text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300">
                    <Menu class="h-5 w-5" />
                </div>
                <div class="flex-1" @click="$router.push(`/board/${board.boardUrl}`)">
                    <div class="text-sm font-medium text-indigo-600 dark:text-indigo-400 truncate">{{ board.boardName }}</div>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ board.description }}</p>
                </div>
            </div>
            <button 
                @click="handleUnsubscribe(board)"
                class="ml-4 px-3 py-1 border border-transparent text-sm font-medium rounded-md text-red-700 bg-red-100 hover:bg-red-200 dark:bg-red-900/50 dark:text-red-400 dark:hover:bg-red-900/70 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
            >
