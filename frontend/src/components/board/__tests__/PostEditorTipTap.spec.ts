...
describe('PostEditorTipTap', () => {
    ...
    it('applies toolbar formatting controls and table insertion', async () => {
        ...
        const colorInput = wrapper.get('#editor-custom-text-color')
    ...
    it('supports list helpers and slash menu actions', async () => {
        ...
        await nextTick()
        const slashButtons = wrapper.findAll('.slash-action-btn')
        expect(slashButtons[slashButtons.length - 1].attributes('tabindex')).toBe('0')
