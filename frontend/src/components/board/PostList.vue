    ...
    <div class="table-container">
      <table class="table-base">
        <thead class="table-head">
          <tr>
            <th v-if="!hideNoColumn" scope="col"
              class="table-th text-center w-24 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors duration-200"
              @click="handleSort('postId')">
              <div class="flex items-center justify-center">
                {{ $t('common.no') }}
                <component v-if="currentSort.startsWith('postId')"
                  :is="currentSort.endsWith('desc') ? ArrowDown : ArrowUp" class="h-4 w-4 ml-1" />
              </div>
            </th>
            ...
            <th scope="col"
              class="table-th text-center w-28 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors duration-200"
              @click="handleSort('createdAt')">
              <div class="flex items-center justify-center">
                {{ $t('common.date') }}
                <component v-if="currentSort.startsWith('createdAt')"
                  :is="currentSort.endsWith('desc') ? ArrowDown : ArrowUp" class="h-4 w-4 ml-1" />
              </div>
            </th>
            <th scope="col"
              ...
              <div class="flex items-center justify-center">
                {{ $t('common.views') }}
                <component v-if="currentSort.startsWith('viewCount')"
                  :is="currentSort.endsWith('desc') ? ArrowDown : ArrowUp" class="h-4 w-4 ml-1" />
              </div>
            </th>
            <th scope="col"
              class="table-th text-center w-24 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors duration-200"
              @click="handleSort('likeCount')">
              <div class="flex items-center justify-center">
                {{ $t('common.likes') }}
                <component v-if="currentSort.startsWith('likeCount')"
                  :is="currentSort.endsWith('desc') ? ArrowDown : ArrowUp" class="h-4 w-4 ml-1" />
              </div>
            </th>
          </tr>
        ...
        <tbody class="table-body">
          <tr v-for="(post, index) in posts" :key="post.postId"
            :class="['table-row', post.isNotice ? 'bg-gray-50 dark:bg-gray-900/50' : '']">
            <td v-if="!hideNoColumn" class="table-td text-center">
              <span v-if="post.isNotice" class="font-bold text-red-600 dark:text-red-400">{{ $t('common.notice')
              }}</span>
              <span v-else>{{ post.rowNum }}</span>
            </td>
            ...
            <td class="table-td font-medium text-gray-900 dark:text-gray-100 align-middle">
              <router-link :to="`/board/${boardUrl || post.boardUrl}/post/${post.postId}`"
                class="hover:text-indigo-600 dark:hover:text-indigo-400 flex items-center h-full"
                v-if="boardUrl || post.boardUrl">
                <span v-if="post.category && post.category.name !== '일반'" class="badge badge-gray">
                  {{ post.category.name }}
                </span>
              ...
              <span v-else class="text-gray-400 flex items-center h-full cursor-not-allowed"
                :title="$t('board.invalidUrl')">
                <span class="truncate">{{ post.title }}</span>
              </span>
            </td>
            <td class="table-td text-center text-gray-900 dark:text-gray-200">
              <UserMenu v-if="post.author" :user-id="post.author.userId" :display-name="post.author.displayName" />
            </td>
