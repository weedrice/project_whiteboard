...
import { createMyAgentsQueryOptions, createMyProfileQueryOptions } from '@/features/user/useUser'
...
export function useMyPageDashboardResource(t: Translate) {
  ...
  const myCommentItems = computed<MyCommentListItem[]>(() => myComments.value.map((comment) => ({
    ...
    postLink: comment.post
      ? buildPostDetailPath(
          comment.post.boardUrl,
          comment.post.postId,
          `#comment-${encodePathSegment(comment.commentId)}`,
        )
      : null,
