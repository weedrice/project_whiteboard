...
describe('adminUserDisplay', () => {
  ...
  it('maps mutable statuses to admin actions', () => {
    ...
    expect(getAdminUserStatusActionLabel(t, 'ACTIVE')).toBe('admin.users.status.SUSPENDED')
    expect(getAdminUserStatusActionLabel(t, 'SUSPENDED')).toBe('admin.users.status.ACTIVE')
  })
})
