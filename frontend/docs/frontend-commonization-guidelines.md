...
- Retired generic classes and `legacy-components.css` must not be reintroduced. The `check:ui` primitive contract rejects the retired filename and references, scans Vue templates (including static, literal/object, and spread class bindings), checks SFC script class literals, and scans CSS/SCSS selectors. Shared `btn-*` and `input-base` classes are owned by their Base primitives and must not be assembled directly in feature templates.
