...
- Route views must use `BaseInput`, `BaseSelect`, `BaseTextarea`, and `BaseTable`/`AdminPaginatedTable`. Outside the base primitive implementations, a native `input`, `select`, `textarea`, or `table` is allowed only for a specialized browser or interaction capability and must declare `data-ui-native="file|color|radio|choice|specialized"`. The declaration must match the native element and input type; use `choice` when one control switches between checkbox and radio behavior.
...
- Retired generic classes and `legacy-components.css` must not be reintroduced. The `check:ui` primitive contract rejects the retired filename and references, resolves static, bound, object, named spread, and multi-line class expressions with the TypeScript AST across Vue and TypeScript sources, and scans CSS/SCSS selectors. Shared `btn-*` and `input-base` classes are owned by their Base primitives and must not be assembled directly in feature templates or helpers.
- Use `BaseTable` with its default standalone appearance when it owns the table surface. Set `appearance="embedded"` when an `AdminPanel` or feature section already owns the border, surface, and shadow.
