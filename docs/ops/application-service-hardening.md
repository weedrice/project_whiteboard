...
The first deployment after installing the active-state verifier must run while the existing backend management health and build-info endpoints are readable. Activation records `/opt/app/backend/app.jar.active.state` as root-owned mode `0600` metadata containing commit, payload and envelope digests, run identity, monotonic run number/attempt, API contract revision, and `pending|stable` phase. systemd rejects a JAR that does not match this state. Non-newer generations require a one-use root-owned mode `0600` break-glass file containing the exact target run number/attempt and an operator reason; never grant the deployment account permission to create that file.
