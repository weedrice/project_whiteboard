...
스키마 변경은 `docs/ops/database-migration-policy.md`의 expand/backfill/application-switch/contract 절차를 함께 따른다. 별도로 승인된 contract migration도 수동 DB snapshot을 생성하지 않으며, RDS 자동 백업과 PITR의 복구 가능 시각을 기준으로 보호한다.
...
- 수동 DB snapshot은 생성하거나 배포 승인 근거로 사용하지 않는다.
...
2. 자동 백업 보존 기간이 7일 이상인지 확인한다.
3. 최근 복구 가능 시각이 RPO 24시간 이내인지 확인한다.
4. 복구 시 사용할 시점과 현재 애플리케이션 artifact를 운영 기록에 남긴다.

자동 백업이 비활성화됐거나 최근 복구 가능 시각이 RPO를 벗어나면 데이터 변경을 포함하는 배포를 진행하지 않는다.
...
2. 기존 운영 RDS를 변경하거나 삭제하지 않은 상태에서 PITR로 새 RDS를 생성한다.
...
분기별 리허설은 운영 트래픽과 분리된 환경에서 수행한다. 검증이 끝나면 삭제 방지를 해제한 뒤 리허설 RDS를 삭제한다.
