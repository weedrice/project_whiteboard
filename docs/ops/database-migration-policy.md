...
Contract migrations are never eligible for an automatic main deployment. Run the integrated CI manually from `main`, explicitly approve the contract phase, and provide a recent manual RDS snapshot. The evidence check assumes a read-only AWS role and fails closed unless the snapshot is `available`, belongs to the configured production DB, predates the deployment, and is no more than 24 hours old. Evidence consumption is a separate one-time state transition performed by a different least-privilege role after activation.

Configure `AWS_CONTRACT_EVIDENCE_READ_ROLE_ARN`, `AWS_CONTRACT_EVIDENCE_CONSUME_ROLE_ARN`, `AWS_REGION`, and `RDS_PRODUCTION_DB_IDENTIFIER` as production-environment secrets. The read role needs only `rds:DescribeDBSnapshots`; the consume role receives only the evidence-state permissions documented with the workflow. Restrict both OIDC roles to the repository's reviewed production environment workflow subject and do not use long-lived AWS access keys for this gate.
...
The risk classifier removes SQL comments and string literals before matching, so examples and stored text cannot authorize or accidentally classify a change. Privilege grants/revocations, default privilege or owner changes, `REASSIGN/DROP OWNED`, trigger or row-level-security policy changes, `SET UNLOGGED`, and partition attachment/detachment are contract operations. Contract snapshot evidence is bound to the exact AWS account, region, production database, encrypted KMS key, PostgreSQL engine major, deployment SHA, and approved change ticket. Successful activation marks the snapshot `noviis:contract-consumed`; a consumed snapshot cannot authorize another contract deployment.
...
Use separate read and consume OIDC roles. `AWS_CONTRACT_EVIDENCE_READ_ROLE_ARN` permits snapshot description and tag reads only. `AWS_CONTRACT_EVIDENCE_CONSUME_ROLE_ARN` permits the same identity lookup plus `rds:AddTagsToResource` only for the approved production snapshot ARN and `noviis:contract-consumed` key. Both trust policies remain restricted to the reviewed production environment subject.

Applied allowlist records and their reviewed evidence manifests are immutable. Add corrections as separately reviewed records instead of rewriting evidence. An allowlist record must reference an existing contract-phase migration, and no new contract evidence record may be added in a change that introduces any migration. This separation prevents an unapplied contract acknowledgement from being bundled with unrelated schema work.
