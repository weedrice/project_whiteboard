...
NoviIs installs the new JAR and starts it; Spring Boot/Flyway applies pending migrations during that application startup. Expand/backfill releases must leave the database usable by both the new application and the previous JAR throughout the rollback window. The inline workflow prepares and verifies the incoming JAR and backup before stopping the service. A failure after stopping but before attempting the new application start restores the verified previous JAR and checks its health, including for contract releases because Flyway has not started. Contract releases are the explicit exception after the new application start is attempted: a failure stops the service without automatically restoring the previous JAR. If stopping or recovery verification fails, the workflow reports failure and requires operator recovery. See the current [deployment workflow contract](../../.github/workflows/README.md).
