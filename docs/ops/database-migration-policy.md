...
The risk classifier removes SQL comments and string literals before matching, so examples and stored text cannot authorize or accidentally classify a change. Privilege revocation, owner changes, `DROP OWNED`, trigger disabling, `SET UNLOGGED`, and partition detachment are contract operations. Contract snapshot evidence is bound to the exact AWS account, region, production database, encrypted KMS key, PostgreSQL engine major, deployment SHA, and approved change ticket. Successful activation marks the snapshot `noviis:contract-consumed`; a consumed snapshot cannot authorize another contract deployment.
