...
- A1~A8, B1~B2, C1, E1~E2, G1~G3는 구현과 계약 테스트까지 완료됐다. A9는 제외 도메인 경계를 기록하는 범위 문서이므로 수정 과제가 아니다.
...
- E3: JPA auditing은 `auditingDateTimeProvider`를 사용하도록 명시됐지만, 도메인·서비스의 `LocalDateTime.now()` 4곳과 JVM 기본 timezone 의존은 남아 있다.
- F1은 공용 유틸리티 API 정리 시, F2는 scale-out 계획 수립 시 다룰 설계 후속이다.
...
| ID | 2026-07-25 발견 항목 | 영역 | 당시 우선순위 | 현재 상태 |
| --- | --- | --- | --- | --- |
| A1 | boolean wire 이름 가드가 일부 DTO만 덮는다 | 계약 | 상 | 완료 |
| A2 | rate limit 응답 헤더를 아무도 읽지 않는다 | 계약 | 상 | 완료 |
| A3 | SSE 이벤트 이름에 단일 출처와 계약 테스트가 없다 | 계약 | 중 | 완료 |
| A4 | `C008`이 두 가지 payload 형태를 가진다 | 계약 | 중 | 완료 |
| A5 | 대상별 업로드 제한이 클라이언트에만 존재한다 | 계약 | 중 | 완료 |
| A6 | 응답 정규화 shim이 6곳에 흩어져 있다 | 계약 | 중 | 완료 |
| A7 | `PageResponseRaw`에 실패 모드가 없다 | 계약 | 하 | 완료 |
| B1 | `validationErrorResponse`의 파라미터 2개가 미사용이다 | 백엔드 | 하 | 완료 |
| B2 | `spring.messages` 설정이 죽어 있고 로케일 fallback 동작이 미확인이다 | 백엔드 | 하 | 완료 |
| C1 | 백엔드 에러 코드 리터럴이 6곳에 분산되어 있다 | 프론트엔드 | 하 | 완료 |
| D1 | HSTS `max-age`가 1일이다 | 배포 | 중 | 7일 상향 완료, 장기화는 운영 판단 |
| E1 | 일일 스케줄러 timezone 명시가 일부 빠져 있다 | 실행 환경 | 하 | 완료 |
| E2 | wire 타임스탬프에 offset이 없어 KST 밖 사용자에게 어긋난다 | 실행 환경 | 상 | 완료 |
| E3 | 시간 기준 통일이 `setDefault` 한 줄에 의존한다 | 실행 환경 | 하 | 부분 완료 |
| F1 | `PageRequestUtils` 오버로드의 두 번째 인자 의미가 충돌한다 | 내부 API | 하 | 설계 후속 |
| F2 | 익명 캐시가 in-process라 scale-out의 선결 과제다 | 실행 환경 | 하 | scale-out 시 후속 |
| G1 | `UserSettings.timezone`이 저장만 되고 쓰이지 않는다 | 기능 | 중 | 완료 |
| G2 | 시각 표시를 사용자 지역 기준으로 전환하는 설계 | 설계 | — | 완료 |
| A8 | 51개 boolean 필드가 wire에 키를 두 개씩 내보낸다 | 계약 | 중 | 완료 |
| A9 | 제외 도메인(agent·ad)에 닿는 변경의 목록 | 범위 | 상 | 경계 기록(수정 과제 아님) |
| G3 | G1/G2 구현에서 검토가 잡아낸 회귀 | 회귀 | 상 | 완료 |
...
> **현재 상태 (2026-08-13)**: 위 문단은 2026-07-25 당시의 판단이다. 이후 wire의 `LocalDateTime`에 `+09:00` offset을 붙이는 직렬화 계약과 사용자 timezone 기반 표시가 구현돼 E2·G1·G2는 완료됐다.

## A. 계약 지점

### A1. boolean wire 이름 가드가 일부 DTO만 덮는다

**현재 상태 (2026-08-13): 완료.** `BooleanWireNameContractTest`와 `BooleanWireRoundTripContractTest`가 대상 DTO의 이름·직렬화 왕복 계약을 검증한다.
...
**현재 상태 (2026-08-13): 완료.** 프론트가 `Retry-After`와 rate limit 헤더를 읽어 재시도 판단 및 오류 메타데이터에 사용하고, 백엔드는 해당 헤더를 CORS에 노출한다.
...
**현재 상태 (2026-08-13): 완료.** 백엔드 `NotificationSseEvents`와 프론트 `notificationSseEvents.ts`가 이벤트 이름을 모으고, 양쪽 계약 테스트가 이름의 일치를 고정한다.
...
**현재 상태 (2026-08-13): 완료.** `ValidationErrorCollector`를 통해 필드 오류 수집과 `details` payload 생성을 통일했고 예외 처리기 테스트로 검증한다.
...
**현재 상태 (2026-08-13): 완료.** 서버가 `FileUploadTarget`별 정책과 association을 검증하므로 클라이언트 제한을 우회해도 대상별 크기 정책이 적용된다.
...
**현재 상태 (2026-08-13): 완료.** 개발 환경에서 `number` 누락을 `warnOnMissingPageNumber`로 드러내고 관련 프론트 테스트가 회귀를 막는다.
...
**현재 상태 (2026-08-13): 완료.** 사용하지 않던 파라미터를 제거해 현재 시그니처는 validation details만 받는다.
...
**현재 상태 (2026-08-13): 완료.** 프론트 에러 코드 리터럴을 `api/errorCodes.ts`의 공통 상수로 모았다.
...
**현재 상태 (2026-08-13): 완료.** `LocalDateTimeWireModule`이 API wire 값에 `+09:00`을 붙이고 API 명세와 직렬화 계약 테스트가 이를 고정한다.
...
**현재 상태 (2026-08-13): 부분 완료.** JPA auditing은 `auditingDateTimeProvider`를 명시적으로 사용한다. 다만 도메인·서비스의 `LocalDateTime.now()` 4곳과 JVM 기본 timezone 의존이 남아 `setDefault` 제거 판단까지는 끝나지 않았다.
...
**현재 상태 (2026-08-13): 완료.** 사용자 설정의 timezone이 프론트 표시 timezone으로 동기화되고 설정 화면에서 `AUTO` 또는 명시 timezone을 선택할 수 있다.
...
**현재 상태 (2026-08-13): 완료.** offset 포함 wire 계약, 공통 날짜 변환, 사용자 timezone 결정과 설정 UI가 연결됐다.
...
### G3. G1/G2 구현에서 검토가 잡아낸 회귀 (조치 완료)

세 커밋(`4a3e318`, `a37b816`, `4a79049`)이 검토 없이 커밋되어 사후 감사를 돌렸다.
`4a79049`에서 **사용자에게 보이는 회귀 2건**이 나왔다. 둘 다 이 작업이 고치려던 바로 그
영역에서 발생했고, 테스트 3385개가 전부 통과하는 상태에서도 드러나지 않았다.

**(1) 새로고침마다 표시 시간대가 지워졌다**

`clearUserTimeZone()`을 `onSessionBoundary`에 걸었는데, 이 경계는 **같은 사용자가 토큰만
새로 받는 경우에도 발생한다.** 부트스트랩(`bootstrapSession` → 쿠키 갱신 →
`applyNewSessionIfCurrent`)이 매 페이지 진입마다 경계를 올리므로, 저장된 지역이 설정
응답이 도착하기 전에 지워졌다.

뉴욕 사용자·계정 설정 `Asia/Seoul` 기준 실패 경로: 진입 시 저장값 삭제 → 목록 렌더 →
`getDisplayTimeZone()`이 브라우저 지역(뉴욕)으로 **기억** → 뒤늦게 설정 응답 도착 →
기억은 비워지지만 **이미 그려진 화면은 다시 그려지지 않는다**. 같은 글이 화면마다 다른
날짜로 보인다. 기억(메모이제이션)이 이를 고착시켰지만, 저장값 삭제만으로도 재현된다.

→ `onPrincipalChange` 효과를 새로 두고 로그인·계정 전환·로그아웃에만 걸었다. 부트스트랩은
`broadcastBoundary=false`로 들어오므로 자연히 제외된다. 다른 탭이 알려 온 로그아웃은
`broadcast=false`여도 이 탭에서는 주체 변경이므로 포함한다.

**(2) "자동"을 저장하면 조용히 실패했다**

`AUTO`를 서버가 거부하므로(`ZoneId.of`) 전송에서 빼도록 고쳤는데, 이것이 더 나쁜 동작을
만들었다. 서버 `updateSettings`는 `if (timezone != null)`이라 값을 빼면 **"변경 없음"으로
읽는다.** 즉 `Asia/Tokyo`를 쓰던 사용자가 "자동"을 골라 "저장되었습니다"를 보고도 서버에는
Tokyo가 그대로 남고, 다음 방문에 Tokyo로 되돌아간다. 눈에 보이던 400을 보이지 않는
실패로 바꾼 셈이다.

여기에 `baseline`을 전송 payload로 잡아 `baseline.timezone === undefined`인데
`form.timezone === 'AUTO'`가 되어 **`isDirty`가 영구히 참**으로 남았다. 저장 버튼이 계속
활성화되고, 하이드레이션 watch가 `if (isDirty) return`으로 막혀 그 세션 동안 서버
재동기화가 아예 되지 않는다(동시 수정 복구 경로 포함).

→ 서버가 `AUTO` 표식을 저장하도록 `normalizeTimezone`에 명시적으로 허용했다. 서버는 이
필드를 판정에 쓰지 않고 저장·응답만 하므로(출석의 "오늘", 일일 한도는 모두 KST 고정)
표식을 담아도 서버 로직에 영향이 없다. 프론트는 `AUTO`를 그대로 보내고, baseline도 폼과
같은 모양으로 잡아 dirty 추적이 어긋나지 않게 했다.

**함께 고친 것**

- 투표 마감 검증이 `datetime-local`이 만든 KST 벽시계를 기기 지역으로 읽었다. 시드니
  사용자가 30분 뒤 마감을 넣으면 "현재보다 이후여야 합니다"로 거부됐다(UTC+14면 최대 5시간).
- 표시 지역 기억이 다른 탭의 변경에 반응하지 않았다. `storage` 이벤트를 듣게 했다.
- `displayZoneParts`가 호출마다 `Intl.DateTimeFormat`을 새로 만들어, 목록 한 화면에서
  formatter를 100번 생성했다. `formatDateShort`는 Intl을 안 쓰던 경로였으므로 순수 악화였다.
  지역별 캐시로 옮겼다.
- 관리자 오류 로그의 기본 날짜 범위가 기기 기준이라, KST와 날짜가 다른 지역의 관리자에게
  서버의 오늘 로그가 통째로 빠졌다.
- 임시저장 시각 칩만 `toLocaleTimeString()`으로 기기 지역이었다.
- `formatTimeAgo`의 7일 초과 fallback이 로케일을 무시하고 한국어로 고정됐다.

**교훈**: 검토를 건너뛴 세 커밋 중 회귀는 전부 가장 큰 커밋 하나(`4a79049`, 8개 변경 묶음)에
있었다. 나머지 두 개는 저위험 항목이었다. 묶음이 클수록 검토 생략의 비용이 커진다.
...
완료 항목을 제외하고 실제로 남은 후속만 정리한다.

1. **E3 잔여 정리** — 도메인·서비스의 `LocalDateTime.now()` 4곳을 주입 시간원으로 전환한 뒤 JVM `setDefault` 유지 필요성을 다시 판단한다.
2. **D1 운영 판단** — HTTPS 전면 운영과 preload 정책을 확인한 뒤 HSTS를 6개월 또는 1년으로 단계적으로 늘릴지 결정한다.
3. **F1 내부 API 정리** — `PageRequestUtils`를 공용 유틸리티로 손보는 시점에 오버로드의 두 번째 인자 의미를 이름과 타입으로 분리한다.
4. **F2 scale-out 준비** — 수평 확장 계획이 확정되면 익명 캐시의 외부 저장소 이전과 일관성 정책을 설계한다.
5. **A9 경계 준수** — agent·ad를 포함하는 후속 변경은 해당 도메인 소유 주체와 별도 계약 검토를 거친다.
