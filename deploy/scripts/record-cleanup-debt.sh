...
case "$debt" in release_retention|incoming_release|status_diagnostic) ;; *) echo "invalid debt type" >&2; exit 64 ;; esac
...
value=0
[ "$action" = set ] && value=1
printf '# HELP noviis_deployment_cleanup_debt Release cleanup requiring operator follow-up.\n' > "$temporary"
